...
### 注意：このように完全修飾クラス名でプログラムを記述すると、コードは読みづらくなるので、インポート宣言をすることで、パッケージ名を省略してクラス名だけで記述できるようにする。

### インポート宣言

外部のモジュールやクラス、関数を利用するために使用される分法構造。異なるソースコードやライブラリから特定の要素を取り込んでそれを利用できるようにする。
...
※java.langパッケージはjavaプログラミン言語の標準ライブラリーで、基本的なデータ型やクラス、例外処理、スレッド、文字列処理など、Javaの基本的な機能を提供するパッケージの一つ。
特別なインポート宣言をする事なく使用する事ができる。

※同じパッケージに属するクラスのインポート宣言も省略する事ができる。
**java.untilパッケージに属する全クラスのインポート宣言**

```java
import java.until*;
```

このようにアスタリスクを使うことで一度に指定したパッケージに属するクラスをインポートできる。サブパッケージはインポートされることは無い。
